...
def runGame():
    ...
    time=0
    hour=0
    gamefont = pygame.font.SysFont("comicsansms", 24)
    """
    창 클래스
    """
    ...
    class menu_model(object):
        def __init__(self,bar,icons,wh):
            self.bar=bar
            self.menu_icons=icons
            self.wh=wh

        def constr(self,d1,d2,d3,menu_on,menu_wifi,hour,min):
            gamepad.blit(self.bar,(0,padHeight-self.wh[1]))
            gamepad.blit(menu_on,(0,padHeight-self.wh[1]))
            gamepad.blit(menu_wifi,(1080,padHeight-self.wh[1]))
            clock_text = []
            virtual_hour=1+hour
            virtual_min=min
            if virtual_min<10:
