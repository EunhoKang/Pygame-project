...
def runGame():
    ...
    6.시간 계산
    """
    def icon_func(icon, window, draw,ins, mouse_x, mouse_y,imp_range):
        icon.constr()
    ...
    def wifi_func(draw, on_off,mouse_x, mouse_y,wait):
        if on_off:
            gamepad.blit(menu_wifi[0], (1080, padHeight - 40))
        else:
            gamepad.blit(menu_wifi[1], (1080, padHeight - 40))
        if event.type == MOUSEBUTTONDOWN and event.button == 1:
            if 1080 <= mouse_x <= 1080+40 and padHeight-40 <= mouse_y <= padHeight:
                draw = True
            elif not (1080+40-256<= mouse_x<=1080+40 and padHeight-40-284 <= mouse_y <= padHeight-40) :
                draw= False
        if draw:
            if wait==1 or wait==3:
                gamepad.blit(menu_wifi[6],(1080+40-256,padHeight-40-284))
            elif on_off:
                gamepad.blit(menu_wifi[2],(1080+40-256,padHeight-40-284))
                gamepad.blit(menu_wifi[4], (1080 + 40 - 256/2-65, padHeight - 40-60))
                if wait==2:
                    if 1080+40-256/2-25<=mouse_x<=1080+40-256/2+25 and padHeight-40-60<=mouse_y<=padHeight-40-60+34:
                        if event.type == MOUSEBUTTONDOWN and event.button == 1:
                            on_off= not on_off
                            wait=3
            else:
                gamepad.blit(menu_wifi[3],(1080+40-256,padHeight-40-284))
                gamepad.blit(menu_wifi[5],(1080 + 40 - 256/2-65, padHeight - 40 -60))
                if wait==0:
                    if 1080+40-256/2-25<=mouse_x<=1080+40-256/2+25 and padHeight-40-60<=mouse_y<=padHeight-40-60+34:
                        if event.type == MOUSEBUTTONDOWN and event.button == 1:
                            on_off= not on_off
    ...
    while not crashed:
        ...
        """
        (icondraw1,iconins1)=icon_func(icon1,window1,icondraw1,iconins1,mouse_x,mouse_y,imp_range)
        (icondraw2,iconins2) = icon_func(icon2, window2, icondraw2,iconins2, mouse_x, mouse_y,imp_range)
        (icondraw3,iconins3) = icon_func(icon3, window3, icondraw3,iconins3, mouse_x, mouse_y,imp_range)
        """
        바를 클릭한 상태에서 창을 움직이는 함수
        """
        if icondraw1 and rec_sub(window1, imp_range, mouse_x, mouse_y):
            (init_x,init_y,windowmove1,windowins1)=window_move(window1,windowmove1,windowins1,init_x,init_y,mouse_x,mouse_y)
        if icondraw2 and rec_sub(window2, imp_range, mouse_x, mouse_y):
            (init_x, init_y, windowmove2,windowins2) = window_move(window2, windowmove2,windowins2, init_x, init_y, mouse_x, mouse_y)
        if icondraw3 and rec_sub(window3, imp_range, mouse_x, mouse_y):
        ...
        if wait==3: #클릭 후 텀을 둬서 반복적으로 클릭되지 못하게 조정
            ...
            if a==10:
                wait=0
                a=0
        if wait==1: #클릭 후 텀을 둬서 반복적으로 클릭되지 못하게 조정
            a=a+1
            if a==10:
                wait=2
                a=0
