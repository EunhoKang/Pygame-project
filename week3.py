...
def runGame():
    ...
    global chrome_on,chrome_off
    global menu,menu_window,menu_icons,menu_on,menu_wifi
    global kakao_logout, kakao_main, kakao_room, kakao_pro
    global none
    global chat_q1, chat_q2, chat_q3, chat_qg, chat_a1, chat_a2, chat_a3, chat_ag,warn,d3msg,new
    global pycharm, pycharm_but
    global pycharm_code,pycharm_code2,pycharm_code3,pycharm_prob,seat,x_mark
    global die
    global glitch_sound, footstep_sound, typing_sound,Gotcha,horray
    global timeover, submit, askquit, askgiveup
    """
    ...
    class menu_model(object):
        ...
        def constr(self,d1,d2,d3,hour,min):
            ...
            gamepad.blit(screen_text2, (1185, padHeight - self.wh[1]/2))
    ...
    def menu_func(draw, mouse_x, mouse_y,command):
    ...
    def kakaochat(Window,mouse_x,mouse_y,draw,drawed,drawob,ins,list):
        ...
        if drawed!=False:
            ...
            if numb==11:
                ...
                drawob=list[4]
            if Window.locate[0]-228+94<=mouse_x<=Window.locate[0]-228+94+40 and Window.locate[1]+272<=mouse_y<=Window.locate[1]+272+40 and rec_sub(Window,mouse_x,mouse_y):
                if event.type == MOUSEBUTTONDOWN and event.button == 1:
                    draw=True
                    ins=True

        if Window.locate[0]+70<=mouse_x<=Window.locate[0]+70+210 and Window.locate[1]+90<=mouse_y<=Window.locate[1]+90+64 and rec_sub(Window,mouse_x,mouse_y):
            if event.type == MOUSEBUTTONDOWN and event.button == 1:
                draw=True
                ins = True
                drawob = list[0]
        if Window.locate[0]+70<=mouse_x<=Window.locate[0]+70+210 and Window.locate[1]+155<=mouse_y<=Window.locate[1]+155+64 and rec_sub(Window,mouse_x,mouse_y):
            if event.type == MOUSEBUTTONDOWN and event.button == 1:
                draw=True
                ins = True
                drawob = list[1]
        if Window.locate[0]+70<=mouse_x<=Window.locate[0]+70+210 and Window.locate[1]+225<=mouse_y<=Window.locate[1]+225+64 and rec_sub(Window,mouse_x,mouse_y):
            if event.type == MOUSEBUTTONDOWN and event.button == 1:
                draw=True
                ins = True
                drawob = list[2]
        if Window.locate[0]+70<=mouse_x<=Window.locate[0]+70+210 and Window.locate[1]+290<=mouse_y<=Window.locate[1]+290+64 and rec_sub(Window,mouse_x,mouse_y):
            if event.type == MOUSEBUTTONDOWN and event.button == 1:
                draw=True
                ins = True
                drawob = list[3]
        if Window.locate[0]+70<=mouse_x<=Window.locate[0]+70+210 and Window.locate[1]+355<=mouse_y<=Window.locate[1]+355+64 and rec_sub(Window,mouse_x,mouse_y):
            if event.type == MOUSEBUTTONDOWN and event.button == 1:
                draw=True
                ins = True
                drawob = list[4]
    ...
    def chatclick(Window,drawob,mouse_x,mouse_y,msg,ins):
        ...
        if drawob.ID==25:
            return msg,ins
        if drawob.ID==23 or drawob.ID==24:
            if Window.locate[0]-drawob.bar[1]+18<=Window.locate[0]-drawob.bar[1]+18+240 and Window.locate[1]+388<=mouse_y<=Window.locate[1]+388+28 and rec_sub(Window,mouse_x,mouse_y):
                if event.type == MOUSEBUTTONDOWN and event.button == 1:
                    msg=1
                    ins=True
            if Window.locate[0]-drawob.bar[1]+18<=Window.locate[0]-drawob.bar[1]+18+240 and Window.locate[1]+424<=mouse_y<=Window.locate[1]+424+28 and rec_sub(Window,mouse_x,mouse_y):
                if event.type == MOUSEBUTTONDOWN and event.button == 1:
                    msg=2
                    ins=True
            if Window.locate[0]-drawob.bar[1]+18<=Window.locate[0]-drawob.bar[1]+18+240 and Window.locate[1]+460<=mouse_y<=Window.locate[1]+460+28 and rec_sub(Window,mouse_x,mouse_y):
                if event.type == MOUSEBUTTONDOWN and event.button == 1:
                    msg=3
                    ins=True
        if drawob.ID==21:
            if Window.locate[0]-drawob.bar[1]+95<=mouse_x<= Window.locate[0]-drawob.bar[1]+95+88 and Window.locate[1]+388<=mouse_y<=Window.locate[1]+388+88 and rec_sub(Window,mouse_x,mouse_y):
                if event.type == MOUSEBUTTONDOWN and event.button == 1:
                    msg=4
                    ins=True
        if msg!=0 and ins:
    ...
    def question(Window,msg,messages,chatdraw,ins,p,change):
        ...
        if Window.locate[0] + 70 <= mouse_x <= Window.locate[0] + 70 + 210 and Window.locate[1] + 90 <= mouse_y <= \
                Window.locate[1] + 90 + 64 and rec_sub(Window,mouse_x,mouse_y):
            if event.type == MOUSEBUTTONDOWN and event.button == 1:
                change=True
        if Window.locate[0] + 70 <= mouse_x <= Window.locate[0] + 70 + 210 and Window.locate[1] + 155 <= mouse_y <= \
                Window.locate[1] + 155 + 64 and rec_sub(Window,mouse_x,mouse_y):
            if event.type == MOUSEBUTTONDOWN and event.button == 1:
                change = True
        if Window.locate[0] + 70 <= mouse_x <= Window.locate[0] + 70 + 210 and Window.locate[1] + 225 <= mouse_y <= \
                Window.locate[1] + 225 + 64 and rec_sub(Window,mouse_x,mouse_y):
            if event.type == MOUSEBUTTONDOWN and event.button == 1:
                change = True
        if Window.locate[0] + 70 <= mouse_x <= Window.locate[0] + 70 + 210 and Window.locate[1] + 290 <= mouse_y <= \
                Window.locate[1] + 290 + 64 and rec_sub(Window,mouse_x,mouse_y):
            if event.type == MOUSEBUTTONDOWN and event.button == 1:
                change = True
        if Window.locate[0] + 70 <= mouse_x <= Window.locate[0] + 70 + 210 and Window.locate[1] + 355 <= mouse_y <= \
                Window.locate[1] + 355 + 64 and rec_sub(Window,mouse_x,mouse_y):
            if event.type == MOUSEBUTTONDOWN and event.button == 1:
                change = True
        if Window.locate[0]+19<=mouse_x<=Window.locate[0]+19+50 and Window.locate[1]+92<=mouse_y<=Window.locate[1]+92+50 and rec_sub(Window,mouse_x,mouse_y):
            if event.type == MOUSEBUTTONDOWN and event.button == 1:
                change = True
        if Window.locate[0]+19<=mouse_x<=Window.locate[0]+19+50 and Window.locate[1]+161<=mouse_y<=Window.locate[1]+161+50 and rec_sub(Window,mouse_x,mouse_y):
            if event.type == MOUSEBUTTONDOWN and event.button == 1:
                change = True
        if Window.locate[0]+19<=mouse_x<=Window.locate[0]+19+50 and Window.locate[1]+226<=mouse_y<=Window.locate[1]+226+50 and rec_sub(Window,mouse_x,mouse_y):
            if event.type == MOUSEBUTTONDOWN and event.button == 1:
                change = True
        if Window.locate[0]+19<=mouse_x<=Window.locate[0]+19+50 and Window.locate[1]+291<=mouse_y<=Window.locate[1]+291+50 and rec_sub(Window,mouse_x,mouse_y):
            if event.type == MOUSEBUTTONDOWN and event.button == 1:
                change = True
        if Window.locate[0]+19<=mouse_x<=Window.locate[0]+19+50 and Window.locate[1]+358<=mouse_y<=Window.locate[1]+358+50 and rec_sub(Window,mouse_x,mouse_y):
            if event.type == MOUSEBUTTONDOWN and event.button == 1:
                change = True
        ...
        if drawob.ID == 22 or drawob.ID == 23 or drawob.ID == 24:
            if Window.locate[0] - drawob.bar[1] + 18 <= Window.locate[0] - drawob.bar[1] + 18 + 240 and Window.locate[
                1] + 388 <= mouse_y <= Window.locate[1] + 388 + 28 and rec_sub(Window,mouse_x,mouse_y):
                if event.type == MOUSEBUTTONDOWN and event.button == 1:
                    change = False
            if Window.locate[0] - drawob.bar[1] + 18 <= Window.locate[0] - drawob.bar[1] + 18 + 240 and Window.locate[
                1] + 424 <= mouse_y <= Window.locate[1] + 424 + 28 and rec_sub(Window,mouse_x,mouse_y):
                if event.type == MOUSEBUTTONDOWN and event.button == 1:
                    change = False
            if Window.locate[0] - drawob.bar[1] + 18 <= Window.locate[0] - drawob.bar[1] + 18 + 240 and Window.locate[
                1] + 460 <= mouse_y <= Window.locate[1] + 460 + 28 and rec_sub(Window,mouse_x,mouse_y):
                if event.type == MOUSEBUTTONDOWN and event.button == 1:
                    change = False
        if drawob.ID == 21:
            if Window.locate[0] - drawob.bar[1] + 95 <= mouse_x <= Window.locate[0] - drawob.bar[1] + 95 + 88 and \
                    Window.locate[1] + 388 <= mouse_y <= Window.locate[1] + 388 + 88 and rec_sub(Window,mouse_x,mouse_y):
                if event.type == MOUSEBUTTONDOWN and event.button == 1:
                    change = False
        if msg>0 and not change:
            if (msg==1 and p<3) or (msg==2 and p<3) or (msg==3 and p<2) or (msg==4 and p==0):
                gamepad.blit(messages[msg-1][p].img, (Window.locate[0] - messages[msg-1][p].len - 3-10, Window.locate[1] + 70))
    ...
    def answer(Window,msg,chatdraw,bool,ins,download,change,p):
        ...
        if msg>0 and (bool==0 or bool==4) and not change:
            if 3+msg+bool>9:
                bool=0
            if (msg+bool==1 and p<3) or (msg+bool==5 and p<3) or (msg+bool==2 and p==0) or (msg+bool==6 and p==0) or (msg+bool==3 and p<7) or (msg+bool==4 and p<2):
                gamepad.blit(messages[3+msg+bool][p].img, (Window.locate[0] - 280 - 3+10, Window.locate[1] + 120))
    ...
    교수님 등장시간
    """
    f_time =random.randrange(11, 20)
    s_time = f_time+random.randrange(20,30)
    t_time = s_time+random.randrange(20,30)
    fo_time=t_time+random.randrange(20,30)
    wifi_time=random.randrange(51,80)
    wifi_time2=random.randrange(91,120)
    ...
    while not crashed:
        ...
        if hour>=2:
            if not endcount and not subcount:
                endcount=True
                print("Time Over!")
                l=t
                m=0
        ...
        for x in imp_range:
            ...
            if x.ID==1:
                if window_type1==1 :
                    if wait==2:
                        ...
                        gamepad.blit(chrome_off, (x.locate[0] + x.window[1] / 2 - 247, x.locate[1] + x.bar[2] + 100))
                elif window_type1==2:
                    gamepad.blit(seat, (x.locate[0]+60, x.locate[1] + x.bar[2]+20))
                    gamepad.blit(x_mark, (x.locate[0] + 60+360, x.locate[1] + x.bar[2] + 20+320))
                elif window_type1==3:
                    gamepad.blit(pycharm_prob[2],(x.locate[0],x.locate[1]+x.bar[2]))
            if x.ID==2 :
                pygame.draw.rect(gamepad,(0,125,60),(x.locate[0]+180,x.locate[1]+46,63,int(270*int(percent)/100)))
                if percent>=100:
                    pygame.draw.rect(gamepad, (0, 225, 0),(x.locate[0] + 180, x.locate[1] + 46, 63, int(270 * int(percent) / 100)))
                How_many=gamefont.render((str(int(percent))+'%'),True,white)
                gamepad.blit(How_many,(x.locate[0]+180+20,x.locate[1]+46+130))
                if 10<=percent<20:
                    gamepad.blit(pycharm_code3[0],(x.locate[0]+180+64,x.locate[1]+46))
                elif 20<=percent<30:
                    gamepad.blit(pycharm_code3[1],(x.locate[0]+180+64,x.locate[1]+46))
                elif 30<=percent<40:
                    gamepad.blit(pycharm_code3[2],(x.locate[0]+180+64,x.locate[1]+46))
                elif 40<=percent<50:
                    gamepad.blit(pycharm_code3[3],(x.locate[0]+180+64,x.locate[1]+46))
                elif 50<=percent<60:
                    gamepad.blit(pycharm_code3[4],(x.locate[0]+180+64,x.locate[1]+46))
                if 60<=percent<70:
                    gamepad.blit(pycharm_code3[5],(x.locate[0]+180+64,x.locate[1]+46))
                elif 70<=percent<80:
                    gamepad.blit(pycharm_code3[6],(x.locate[0]+180+64,x.locate[1]+46))
                elif 80<=percent<90:
                    gamepad.blit(pycharm_code3[7],(x.locate[0]+180+64,x.locate[1]+46))
                elif 90<=percent<100:
                    gamepad.blit(pycharm_code3[8],(x.locate[0]+180+64,x.locate[1]+46))
                elif 100<=percent:
                    gamepad.blit(pycharm_code3[9],(x.locate[0]+180+64,x.locate[1]+46))
                    if x.locate[0] + 793 <= mouse_x <= x.locate[0] + 886 and x.locate[1]+337<=mouse_y<=x.locate[1]+484:
                        if event.type == MOUSEBUTTONDOWN and event.button == 1:
                            if not endcount:
                                subcount = True
                                pygame.mixer.Sound.play(horray)
                                pygame.mixer.Sound.set_volume(horray, 0.5)
                                print("submitted")
                                l = t
                                m = 0
            ...
            if x.ID==3 :
                if wait == 2:
                    window3.window[0] = kakao_main
                else:
                    window3.window[0] = kakao_logout
                if wait==2:
                    if msg == 1:
                        if drawob.ID==23:
                            if t<f_time+10:
                                if f_time-10<=t<=f_time+10 :
                                    bool=4
                                    sorry3 = True
                                else:
                                    bool=0
                                    sorry3 = False
                            elif f_time+10<=t<=s_time+10:
                                if s_time-10<=t<=s_time+10:
                                    bool=4
                                    sorry3 = True
                                else:
                                    bool=0
                                    sorry3 = False
                            elif s_time+10<=t<=t_time+10:
                                if t_time-10<=t<=t_time+10:
                                    bool=4
                                    sorry3 = True
                                else:
                                    bool=0
                                    sorry3 = False
                            elif t_time+10<=t<=fo_time+10:
                                if fo_time-10<=t<=fo_time+10:
                                    bool=4
                                    sorry3 = True
                                else:
                                    bool=0
                    ...
                    if drawob.ID==25 and drawchat:
                        if deathcount>0:
                            for g in range(deathcount):
                                gamepad.blit(warn,(x.locate[0]-280+3,x.locate[1]+3+64+31*g))
                    if wait == 2:
                        if drawob.ID == 22 and drawchat:
                            gamepad.blit(d3msg, (x.locate[0] - 280 + 3, x.locate[1] + 3 + 64))
                    if 3+msg+bool==5 and downtemp and drawchat :
                        if x.locate[0]-280+3<=mouse_x<=x.locate[0]-280+3+180 and x.locate[1]+3+100<=mouse_y<=x.locate[1]+3+100+74:
                            if event.type == MOUSEBUTTONDOWN and event.button == 1:
                                downcount=True
                                downtemp=False
                                c=t
                                downpercent=0
                    if drawchat and not changeop and not changean and msg==2:
                        if downcount:
                            if t-c>=0.25:
                                downpercent+=0.5
                            if downpercent<100:
                                pygame.draw.rect(gamepad,(0,125,60),(x.locate[0]-280+10,x.locate[1]+100+70,downpercent,8))
                            if downpercent>=100:
                                downpercent=100
                                pygame.draw.rect(gamepad, (0, 225, 0),
                                                 (x.locate[0] - 280 + 10, x.locate[1] + 100 + 70, downpercent, 8))

                                if x.locate[0] - 280 + 3 <= mouse_x <= x.locate[0] - 280 + 3 + 180 and x.locate[
                                    1] + 3 + 100 <= mouse_y <= x.locate[1] + 3 + 100 + 74 and rec_sub(window3,mouse_x,mouse_y):
                                    if event.type == MOUSEBUTTONDOWN and event.button == 1:
                                        if percent<75:
                                            percent+=25
                                        else:
                                        ...
                                        downcount = False
                                        downtemp=True
                                        imp_insert(window2,'ins')
                                        icondraw2=True
                                        downpercent=0
                                        drawchat=False
                    if changeop or changean or not wifi_onoff or not msg==2:
                        downcount = False
                        downtemp = True
                        downpercent = 0
                if new_msg2:
                    gamepad.blit(new,(x.locate[0]+60,x.locate[1]+360))
                    if drawob.ID==25 and drawchat:
                        new_msg2 = False
                if new_msg3:
                    gamepad.blit(new,(x.locate[0]+60,x.locate[1]+160))
                    if drawob.ID==22 and drawchat:
                        new_msg3 = False
        if not icondraw3:
            drawchat = False
        ...
        if imp_range!=[] and percent<100 :
            if imp_range[-1].ID==1 and window_type1==1 and wifi_onoff:
                if not count:
                    b = t
                    count = True
                if t-b>=0.5:
        ...
        메뉴 전원
        """
        (draw_on, command) = menu_func(draw_on, mouse_x, mouse_y, command)
        """
        메뉴 와이파이
        """
        ...
        교수님이 올 시간 & 데스씬
        """
        if (t == f_time or t == s_time or t == t_time) and imp_range != []:
            for imp in imp_range:
                if imp == window1 or imp == window3:
                    gotcha = True
            if gotcha:
                if not scenecount:
        ...
        if scenecount:
            ...
            if h > 4:
                ...
                new_msg1=True
                new_msg2=True
                imp_insert(window1, "rem")
                imp_insert(window3, "rem")
        ...
        효과음
        """
        if (t == f_time-1 or t == s_time-1 or t == t_time-1 or t==fo_time-1):
            if not soundcount:
                pygame.mixer.Sound.play(footstep_sound)
                pygame.mixer.Sound.set_volume(footstep_sound,0.2)
                soundcount=True
                j=t
                a=0
        if soundcount:
        ...
        if command=="give up" and not endcount and not subcount:
            gamepad.blit(askgiveup,(0,0))
            if event.type==MOUSEBUTTONDOWN and event.button==3:
                print("give up")
                pygame.mixer.music.stop()
                with open("Data/PF.txt", "w", encoding='utf8') as f:
                    f.write("0")
                    f.write("\n")
                crashed=True
            elif not (0 <= mouse_x <= 246 and padHeight - 40 - 162 <= mouse_y <= padHeight - 40):
                if event.type==MOUSEBUTTONDOWN and event.button==1:
                    command=None
        elif command=="exit" and not endcount and not subcount:
            gamepad.blit(askquit,(0,0))
            if event.type==MOUSEBUTTONDOWN and event.button==3:
                print("exit")
                pygame.mixer.music.stop()
                crashed=True
...
def initGame():
    ...
    pygame.display.set_caption('3주차 실습')#제목 설정
    ...
    chrome_off=pygame.image.load("image/chrome_offline.png")

    seat=pygame.image.load("image/seat.png")
    """
    카카오톡
    """
    ...
    d3msg=pygame.image.load("image/day3_f1.png")

    warn=pygame.image.load("image/warn.png")
    new = pygame.image.load("image/new.png")
    """
    파이참
    """
    ...
    pycharm_code=[]
    pycharm_code.append(pygame.image.load("image/day1_1.png"))
    pycharm_code.append(pygame.image.load("image/day1_2.png"))
    pycharm_code.append(pygame.image.load("image/day1_3.png"))
    pycharm_code.append(pygame.image.load("image/day1_4.png"))
    pycharm_code.append(pygame.image.load("image/day1_5.png"))
    pycharm_code.append(pygame.image.load("image/day1_6.png"))
    pycharm_code.append(pygame.image.load("image/day1_7.png"))
    pycharm_code.append(pygame.image.load("image/day1_8.png"))
    pycharm_code.append(pygame.image.load("image/day1_9.png"))
    pycharm_code.append(pygame.image.load("image/day1_10.png"))

    pycharm_code2 = []
    pycharm_code2.append(pygame.image.load("image/day2_1.png"))
    pycharm_code2.append(pygame.image.load("image/day2_2.png"))
    pycharm_code2.append(pygame.image.load("image/day2_3.png"))
    pycharm_code2.append(pygame.image.load("image/day2_4.png"))
    pycharm_code2.append(pygame.image.load("image/day2_5.png"))
    pycharm_code2.append(pygame.image.load("image/day2_6.png"))
    pycharm_code2.append(pygame.image.load("image/day2_7.png"))
    pycharm_code2.append(pygame.image.load("image/day2_8.png"))
    pycharm_code2.append(pygame.image.load("image/day2_9.png"))
    pycharm_code2.append(pygame.image.load("image/day2_10.png"))

    pycharm_code3 = []
    pycharm_code3.append(pygame.image.load("image/day3_1.png"))
    pycharm_code3.append(pygame.image.load("image/day3_2.png"))
    pycharm_code3.append(pygame.image.load("image/day3_3.png"))
    pycharm_code3.append(pygame.image.load("image/day3_4.png"))
    pycharm_code3.append(pygame.image.load("image/day3_5.png"))
    pycharm_code3.append(pygame.image.load("image/day3_6.png"))
    pycharm_code3.append(pygame.image.load("image/day3_7.png"))
    pycharm_code3.append(pygame.image.load("image/day3_8.png"))
    pycharm_code3.append(pygame.image.load("image/day3_9.png"))
    pycharm_code3.append(pygame.image.load("image/day3_10.png"))

    pycharm_prob=[]
    pycharm_prob.append(pygame.image.load("image/day1.png"))
    pycharm_prob.append(pygame.image.load("image/day2.png"))
    pycharm_prob.append(pygame.image.load("image/day3.png"))

    die=pygame.image.load("image/die.png")
    x_mark=pygame.image.load("image/x_mark.png")

    """
    효과음
    """
    glitch_sound=pygame.mixer.Sound("musics/Glitch.wav")
    Gotcha= pygame.mixer.Sound("musics/gotcha.wav")
    footstep_sound=pygame.mixer.Sound("musics/Footstep.wav")
    typing_sound=pygame.mixer.Sound("musics/Keyboard_Typing.wav")
    horray = pygame.mixer.Sound("musics/horray.wav")

    timeover = pygame.image.load("image/timeover.png")
    submit = pygame.image.load("image/submit.png")
    askquit = pygame.image.load("image/askquit.png")
    askgiveup = pygame.image.load("image/askgiveup.png")

    """
    런게임 실행
    """
    pygame.mixer.music.load('musics/Keyboard_Typing.wav')
