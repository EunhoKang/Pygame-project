...
def runGame():
    ...
    global none
    global chat_q1, chat_q2, chat_q3, chat_qg, chat_a1, chat_a2, chat_a3, chat_ag,warn
    global pycharm, pycharm_but
    global pycharm_code,pycharm_code2,pycharm_code3,pycharm_prob,seat
    global die,die2
    global glitch_sound, footstep_sound, typing_sound,horray
    ...
    def kakaomain(Window,mouse_x,mouse_y,draw,ins,list):
        ...
        if draw!=False :
            if Window.locate[0]-draw.buttonx[1] <= mouse_x <= Window.locate[0] and Window.locate[1] <= mouse_y <= Window.locate[1] + draw.buttonx[2] and rec_sub(Window,mouse_x,mouse_y) :
                if event.type == MOUSEBUTTONDOWN and event.button == 1:
                    #imp_insert(draw,'rem')
                    draw = False
    ...
    window3 = window_model([kakao_main, 280, 496], [none, 280, 80], [none, 20, 20],[none,20,20],[window1_loc[0] + 600, window1_loc[1]],3)
    ...
    while not crashed:
        ...
        for x in imp_range:
            ...
            x.constr()
            if x.ID==1:
                if prologue2 and icondraw1:
                    gamepad.blit(click2,(x.locate[0]+235,x.locate[1]-80))
                    if window_type1==3:
                        prologue2=False
                if window_type1==1 :
                    if wait==2:
                    ...
                    gamepad.blit(pycharm_prob[0],(x.locate[0],x.locate[1]+x.bar[2]))
            if x.ID==2 :
...
def initGame():
    ...
    pygame.display.set_caption('1주차 실습')#제목 설정
    ...
    click2=pygame.image.load("image/click2.png")
    """
    런게임 실행
    """
