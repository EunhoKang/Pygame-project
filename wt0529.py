...
def runGame():
    ...
    def window_move(Window,move,ins,init_x,init_y,mouse_x,mouse_y,type):
        ...
        if Window.locate[0]<=mouse_x<=Window.locate[0]+Window.bar[1]-Window.buttonx[1]-Window.buttonm[1] and Window.locate[1] <= mouse_y<=Window.locate[1]+Window.buttonx[2] and not(Window.locate[0]<=mouse_x<=Window.locate[0]+106+106+106 and Window.locate[1]+8<=mouse_y<=Window.locate[1]+8+28):
            if event.type==MOUSEBUTTONDOWN and event.button==1:
                move=True
                ins=True
                (init_x, init_y) = pygame.mouse.get_pos()
